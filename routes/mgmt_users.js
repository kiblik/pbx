...
router.get('/', function(req, res, next) {
  ...
  pc(ip,function(perm){
    ...
    connection.query('SELECT u.ID, INET_NTOA(Net_ID) as Net_ID, Net_mask, ID_Group, ID_Phone, p.Description as pDes, u.Description FROM `Users` as u LEFT JOIN Phones as p ON p.ID = u.ID_Phone', function(err, rows, fields) {
      if (!err){      
